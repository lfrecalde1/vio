...
class MSCKF(object):
    def __init__(self, config):
        ...
        self.state_server.state_cov[3:6, 3:6] = self.config.gyro_bias_cov
        self.state_server.state_cov[6:9, 6:9] = self.config.velocity_cov
        self.state_server.state_cov[9:12, 9:12] = self.config.acc_bias_cov
        self.state_server.state_cov[15:18, 15:18] = self.config.extrinsic_rotation_cov
        self.state_server.state_cov[18:21, 18:21] = self.config.extrinsic_translation_cov

        self.state_server.imu_state.id = np.int64(0)
        ...
        self.state_server.imu_state.R_cam0_imu = T_cam0_imu[:3, :3].T
        self.state_server.imu_state.t_cam0_imu = T_cam0_imu[:3, 3]


        ## Init state_server imu gyro bias
        self.state_server.imu_state.gyro_bias = np.zeros((3, ), dtype=np.double)
        ...
        self.R_cam0_cam1 = CAMState.R_cam0_cam1
        self.t_cam0_cam1 = CAMState.t_cam0_cam1

        IMUState.T_imu_body = Isometry3d(
            config.T_imu_body[:3, :3],
            config.T_imu_body[:3, 3])
        
        T_cam1_imu = np.linalg.inv(config.T_imu_cam1)
        self.R_cam1_imu = T_cam1_imu[:3, :3].T
    ...
    def imu_callback(self, imu_msg, ground_true_msg):
        """
        ...
        if not self.is_gravity_set:
            if len(self.imu_msg_buffer) >= 200:
                ...
                if ground_true_msg is not None:
                    t = ground_true_msg.p
                    v = ground_true_msg.v
                    self.state_server.imu_state.position = t
    ...
    def initialize_gravity_and_bias(self):
        print("init Bias and Gravity")
        # get IMU data up to this point
        angular_velocity_array = np.array([msg.angular_velocity for msg in self.imu_msg_buffer])
        linear_acceleration_array = np.array([msg.linear_acceleration for msg in self.imu_msg_buffer])

        # Vectorized sum across rows (i.e., sum over the message list)
        angular_velocity_sum = np.sum(angular_velocity_array, axis=0)
        linear_acceleration_sum = np.sum(linear_acceleration_array, axis=0)

        # Set imu gyro bias base on the average
        self.state_server.imu_state.gyro_bias = (angular_velocity_sum)/len(self.imu_msg_buffer)

        #set gravity IMU
        gravity_imu = (linear_acceleration_sum)/len(self.imu_msg_buffer)

        # This is the gravity
        gravity_norm = np.linalg.norm(gravity_imu)
        IMUState.gravity = np.array([0, 0, -gravity_norm], dtype=np.double)

        q0_iw = from_two_vectors(-IMUState.gravity, gravity_imu)
        R0_iw = to_rotation(q0_iw)
        #self.state_server.imu_state.orientation = to_quaternion(R0_iw.T)
        self.state_server.imu_state.orientation = q0_iw
        return None
    ...
    def batch_imu_processing(self, time_bound):
        ...
        for msg in self.imu_msg_buffer:
            ...
            if imu_time < self.state_server.imu_state.timestamp:
                used_imu_msg_cntr = used_imu_msg_cntr + 1
                continue
            if imu_time > time_bound:
                break
            # Get angular velocity and acceleration from the buffter
            m_gyro = msg.angular_velocity
            m_acc = msg.linear_acceleration
        ...
        print(self.state_server.imu_state.id)
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        F = np.zeros((21, 21), dtype=np.double)
        G = np.zeros((21, 12), dtype=np.double)

        orientation_quaternion = R.from_quat(self.state_server.imu_state.orientation)
        # This is the rotation matrix as R
        rotation = orientation_quaternion.as_matrix()
        ...
        # Aproximation of matrix exponential
        Fdt = F*dt
        Fdt_square = Fdt @ Fdt
        ...
        PHI = np.eye(21) + Fdt + (1/2)*Fdt_square + (1/6.0)*Fdt_cube
        ...
        q_null_object = R.from_quat(self.state_server.imu_state.orientation_null)
        R_kk_1 = to_rotation(imu_state.orientation_null)

        q_object = R.from_quat(self.state_server.imu_state.orientation)
        R_t = q_object.as_matrix()
        ...
        self.state_server.imu_state.position_null = self.state_server.imu_state.position.copy()
        self.state_server.imu_state.velocity_null = self.state_server.imu_state.velocity.copy()
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        g = self.state_server.imu_state.gravity
        
        # Compute the dq_dt, dq_dt2 in equation (1) in "MSCKF" paper
        # Precompute quaternion propagation
        if gyro_norm > 1e-5:
            dq_dt = (np.cos(gyro_norm * dt * 0.5) * np.eye(4) +
                     (1 / gyro_norm) * np.sin(gyro_norm * dt * 0.5) * Omega) @ q
            dq_dt2 = (np.cos(gyro_norm * dt * 0.25) * np.eye(4) +
                      (1 / gyro_norm) * np.sin(gyro_norm * dt * 0.25) * Omega) @ q
            dq_dt = R.from_quat(dq_dt)
            dq_dt2 = R.from_quat(dq_dt2)

            dR_dt = dq_dt.as_matrix()
            dR_dt2 = dq_dt2.as_matrix()
        else:
        ...
        orientation_quaternion = R.from_quat(q)
        R_matrix = orientation_quaternion.as_matrix()


        # RK4 integration
        k1_v_dot = R_matrix @ acc + g
        k1_p_dot = v


        k1_v = v + 0.5 * dt * k1_v_dot
        k2_v_dot = dR_dt2 @ acc + g
        k2_p_dot = k1_v

        k2_v = v + 0.5 * dt * k2_v_dot
        k3_v_dot = dR_dt2 @ acc + g
        k3_p_dot = k2_v

        k3_v = v + dt * k3_v_dot
        k4_v_dot = dR_dt @ acc + g
        k4_p_dot = k3_v

        # Update state
        self.state_server.imu_state.orientation = quaternion_normalize(dq_dt.as_quat())
        self.state_server.imu_state.velocity = v + (dt / 6) * (k1_v_dot + 2 * k2_v_dot + 2 * k3_v_dot + k4_v_dot)
        self.state_server.imu_state.position = p + (dt / 6) * (k1_p_dot + 2 * k2_p_dot + 2 * k3_p_dot + k4_p_dot)
        return None
    ...
    def state_augmentation(self, time):
        ...
        # Camera 0 estrinsic parameters
        R_cam_imu = self.state_server.imu_state.R_cam0_imu
        ...
        t_camera_world = self.state_server.imu_state.position + R_t@t_cam_imu
        R_camera_world = R_t@R_cam_imu
        R_camera_world = R.from_matrix(R_camera_world)

        cam_id = self.state_server.imu_state.id
        self.state_server.cam_states[cam_id] = CAMState(cam_id)
        cam_state = self.state_server.cam_states[cam_id]


        cam_state.timestamp = time
        cam_state.orientation = R_camera_world.as_quat()  # Save as xyzw
        ...
        J = np.zeros((6, 21), dtype=np.double)
        J[0:3, 0:3] = R_cam_imu.T
        J[0:3, 15:18] = np.eye(3)
        J[3:6, 0:3] = skew(R_t@t_cam_imu)
    ...
    def measurement_jacobian(self, cam_state_id, feature_id):
        ...
        # Cam0 pose.
        q_w_c0 = R.from_quat(cam_state.orientation)
        R_w_c0 = q_w_c0.as_matrix()
        t_c0_w = cam_state.position

        # Cam1 respect to IMU
        R_cam1_imu = self.R_cam1_imu
        t_cam1_imu = self.t_cam1_imu

        # IMu Rotation matrix
        q_object = R.from_quat(self.state_server.imu_state.orientation)
        R_t = q_object.as_matrix()

        t_c1_w = self.state_server.imu_state.position + R_t@t_cam1_imu
        R_camera1_world = R_t@R_cam1_imu
        R_w_c1 = R_camera1_world
        ...
        p_c0 = R_w_c0.T @ (p_w - t_c0_w)
        p_c1 = R_w_c1.T @ (p_w - t_c1_w)
        ...
        A = H_x   # shape: (4, 6)
        u = np.zeros(6)
        u[:3] = to_rotation(cam_state.orientation_null).T @ IMUState.gravity
    ...
    def measurement_update(self, H, r):
        ...
        """
        # Optionally thin H if rows > columns
        if H.shape[0] > H.shape[1]:
            Q, R_aux = np.linalg.qr(H, mode='reduced')
            H_thin = R_aux
            r_thin = Q.T @ r
        else:
            H_thin = H
        ...
        R_cov = self.config.observation_noise * np.eye(H_thin.shape[0])
        ...
        K = np.linalg.solve(S, H_thin @ P).T
            ...
            # Compute the error state delta_x
        delta_x = K @ r_thin
        ...
        if (np.linalg.norm(delta_x_imu[6:9]) > 0.5 or np.linalg.norm(delta_x_imu[12:15]) > 1.0):
            print("[WARN] delta velocity:", np.linalg.norm(delta_x_imu[6:9]))
            print("[WARN] delta position:", np.linalg.norm(delta_x_imu[12:15]))
            print("[WARN] Large state correction detected.")

        dq_imu = small_angle_quaternion(delta_x_imu[0:3])
        self.state_server.imu_state.orientation = quaternion_multiplication(
             self.state_server.imu_state.orientation, dq_imu)
        ...
        R_extrinsic = R.from_quat(dq_extrinsic).as_matrix()
        self.state_server.imu_state.R_cam0_imu = self.state_server.imu_state.R_cam0_imu @ R_extrinsic
